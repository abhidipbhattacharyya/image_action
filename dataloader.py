...
class Raw_dataset:
    def __init__(self, flickrfile, label_list, top_labels = 3):
        self.data = json.load(open(flickrfile, 'r'))
        self.label_list = []

        with open(label_list, 'r') as l:
            ll = l.readlines()

        self.label_list = [l.strip().split('\t')[0] for l in ll]
        self.label_list.append('UNK') #tag for unknown
        self.label_map= {t:i for i,t in enumerate(self.label_list)}

    def label_to_id(self, label):
        if label not in self.label_map:
            return self.label_map['UNK']
        else:
...
class MIL_dataset(Dataset):
    ...
    def convert_single_example (self, datapoint):
        ...
        labels = datapoint['Prop_labels']

        for i, (l,v) in enumerate(labels.items()):
            if label_hot_vec[ self.raw_dataset.label_to_id(l)] == 0 and self.raw_dataset.get_label(l) !='UNK': # to avoid UNK multiple time and other repeataion
                #self.data_stat.update([self.raw_dataset.get_label(l)])
                label_hot_vec[ self.raw_dataset.label_to_id(l)]= 1#1
    ...
    def __getitem__(self, index):
        ...
        if self.tsv == True:
            if mode == 'test':
                ...
                return torch.FloatTensor(transformed_image ), torch.FloatTensor(f_i['tsv_feat']), torch.FloatTensor(boxes),  torch.FloatTensor(interaction_pattern),torch.FloatTensor(f_i['label_hot_vec'])
        else:
            if self.mode == 'test':
